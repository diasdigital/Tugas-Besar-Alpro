...
func printArrwisata(T tabWisata, n int) {
	...
	var headerTabel [6]string
	var lebarHeaderTabel [6]int
	var lebar [6]int
	for i := 0; i < n; i++ {
		row = T[i]
	...
	headerTabel = [6]string{"Nama", "Kota", "Harga", "Hari Buka", "Jam Buka", "Kontak"}
	lebarHeaderTabel = [6]int{4, 4, 5, 9, 8, 6}

	for i := 0; i < len(headerTabel); i++ {
		if lebar[i] < lebarHeaderTabel[i] {
			lebar[i] = lebarHeaderTabel[i]
		}
	}
	...
	for i := 0; i < len(lebar); i++ {
		fmt.Printf("| %-*s ", lebar[i], headerTabel[i])
	}
	fmt.Printf("|\n")
